...
public class Main extends Application {
	...
	public void start(Stage primaryStage /* which is the main window */) {
		try {
			...
			// for the sake of fun, get the primary screen size and set the
			// window to be that size without "maximizing" it
			final Rectangle2D primaryScreenBounds = Screen.getPrimary()
					.getVisualBounds();

			// scene containing the root layout
			final Scene scene = new Scene(root, primaryScreenBounds.getWidth(),
			...
			final Circle biggest = new Circle(
					primaryScreenBounds.getHeight() / 3);
	...
	private List<Circle> generateCircles(final Scene scene) {
		...
		ExecutorService executors = Executors.newFixedThreadPool(NUMBER_THREAD_AVAILABLE);

		for (int i = 0; i < NUMBER_THREAD_AVAILABLE; i++) {
			//TODO improve number of circle for thread
			Runnable runner = new CircleFactory(NCIRCLES / NUMBER_THREAD_AVAILABLE,circles ,scene);
	...
	// draws the circles intersecting the main figure (at this stage it's a
	// circle)
	...
	private void daltonize(final Circle c) {
		if (this.toggle)
			c.setFill(new Color(141. / 255, 161. / 255, 222. / 255, 0.9)); // hardcoded
																			// color
																			// value
		else
	...
	static double distance(Circle a, final Circle b) {
		final double centerXsquared = Math.pow(a.getCenterX() - b.getCenterX(),
				2);
		final double centerYsquared = Math.pow(a.getCenterY() - b.getCenterY(),
				2);
		return Math.sqrt(centerXsquared + centerYsquared);
	}

	// random circles factory
	public static Circle circleFactory(double width, double height) {
	...
	public boolean Intersects(Circle a, Circle b) {
		...
		// before doing an intense calculation quickly check if they're far
		// apart
		if (minrad + maxrad < distance)
			return false;

		// do intense calculations
		if (distance < Math.abs(maxrad + minrad)
				&& distance(a, b) > Math.abs(maxrad - minrad))
			return true;
	...
	public static Color randomColor() {
		/*
		 * colors to use: 0;65;1 //Dark green 75;56;0 //Dark brown 141;161;222
		 * //bluish 169;148;224 //pinkish -------- 0;174;0 //green 110;0;0
		 * //Dark red ------------------ 31;31;248 //Blue 255;34;51 //red
		 * 119;123;140
		 */
		// RGB values
		...
		// final double O = ThreadLocalRandom.current().nextDouble(MINOPACITY,
		// MAXOPACITY);
